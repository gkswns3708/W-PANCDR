...
def train_W_PANCDR_nested(
    ...
    for outer_fold, (train_idx, test_idx) in enumerate(outer_skf.split(X_drug_feat_data, Y)):
        ...
        final_model = W_PANCDR(outer_train_data, outer_test_data)
        
        # weight 저장 경로 설정 (예시)
        weight_path = f'../checkpoint/FC/kfold/W-model_Lnz_best_outerfold_{outer_fold}.pt'
...
if __name__ == "__main__":
    ...
    # 결과 데이터프레임 저장 경로 설정
    result_save_path = os.path.join(dir_path, 'FC_Lnz_results.csv')
