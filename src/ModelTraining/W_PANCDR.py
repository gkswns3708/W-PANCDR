...
class train_W_PANCDR():
        ...
        for epoch in tqdm(range(1000), desc="Epoch", leave=True):
            for i, data in enumerate(tqdm(zip(GDSC_Loader, cycle(E_TEST_Loader)), desc=f"Batch (Epoch {epoch})", leave=False, total=len(GDSC_Loader))):
                ...
                with torch.no_grad():
                    ...
                    F_gexpr,_,_ = EN_model(gexpr)      # fake (GDSC 라고 가정)
                    F_t_gexpr,_,_ = EN_model(t_gexpr)  # real (TCGA 라고 가정)
                ...
                # 1) latent 추출 (Encoder)
                g_latents = EN_model(gexpr)[0]    # fake sample
                t_latents = EN_model(t_gexpr)[0]  # real sample
    ...
    def predict(self, data, params, weight_path):
        ...
        return y_pred.cpu()
# TODO: 이후에 사용시에 다시 refactoring 할 것.
...
#         optimizer = torch.optim.Adam(itertools.chain(EN_model.parameters(),GCN_model.parameters()), lr=lr)
#         # TODO: 변수명 수정 예정.
#         optimizer_adv = torch.optim.Adam(Critic_model.parameters(), lr=lr_critic)
