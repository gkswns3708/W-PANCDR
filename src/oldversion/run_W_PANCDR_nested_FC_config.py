...
lr_ls =        [0.001, 0.0001]
lr_critic_ls = [0.001, 0.0001] # lr_critic도 존재한다고 가정
lam_ls = [1, 0.1, 0.01]
...
def train_W_PANCDR_nested(
    ...
    for outer_fold, (train_idx, test_idx) in enumerate(outer_skf.split(X_drug_feat_data, Y)):
        ...
        # 여기서의 test_t는 outer fold test set을 의미
        ...
        weight_path = os.path.join(config['weight_path'], f'W-model_best_outerfold_{outer_fold}.pt')
...
if __name__ == "__main__":
    ...
    Drug_info_file = config['Drug_info_file']
    Cell_line_info_file = config['Cell_line_info_file']
    Drug_feature_file = config['Drug_feature_file'] 
    Cancer_response_exp_file = config['Cancer_response_exp_file']
    Gene_expression_file = config['Gene_expression_file']
    P_Gene_expression_file = config['P_Gene_expression_file']
    T_Drug_info_file = config['T_Drug_info_file']
    T_Patient_info_file = config['T_Patient_info_file']
    T_Drug_feature_file = config['T_Drug_feature_file']
    T_Cancer_response_exp_file = config['T_Cancer_response_exp_file']
    T_Gene_expression_file = config['T_Gene_expression_file']
    Max_atoms = 100
    ...
    result_df = train_W_PANCDR_nested(
        ...
        n_random_search=config['n_random_search'],
    ...
    result_save_path = os.path.join(config['weight_path'], 'total_results.csv')


    # 상위 디렉토리 추출
    dir_path = os.path.dirname(result_save_path)
